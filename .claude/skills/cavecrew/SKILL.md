...
Origin File: skills/cavecrew/SKILL.md
...
| Review diff, branch, or file for bugs (auto-review after BUILD) | `cavecrew-reviewer` |
| User asks "review this PR / review the diff" | `caveman-review` skill (inline) |
| Deep code review with rationale + alternatives | Main thread (prose review) |
| Write unit tests for a file or function | `cavecrew-tester` |
...
- Don't ask `cavecrew-reviewer` for "general feedback" — it returns findings only, no architecture opinions. Use main thread for architecture review.
