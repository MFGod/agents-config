...
# Origin File: skills/caveman-compress/scripts/validate.py
...
def extract_urls(text):
    return {url.rstrip('.,!?;:)>]') for url in URL_REGEX.findall(text)}


def extract_paths(text):
    text_clean = text
    for block in extract_code_blocks(text):
        text_clean = text_clean.replace(block, "", 1)
    text_clean = re.sub(r'`[^`]+`', '', text_clean)
    return set(PATH_REGEX.findall(URL_REGEX.sub('', text_clean)))
...
def extract_inline_codes(text):
    ...
    for block in extract_code_blocks(text):
        text_clean = text_clean.replace(block, "", 1)
    return re.findall(r"`([^`]+)`", text_clean)
