 ...
 * Origin File: src/hooks/caveman-stats.js
...
// usage plus an estimated savings figure derived from a measured compression
// ratio (see COMPRESSION below).
//
...
//
// FROZEN: lifetime analytics — low priority, do not extend.
...
// Mean per-task output-token reduction for each caveman mode, measured against
// an uncompressed baseline (sonnet-4, 10-task sample). Only 'full' is
// calibrated; add an entry when another mode is benchmarked.
...
function formatStats({ outputTokens, cacheReadTokens, turns, mode, model, sessionPath, compressed }) {
  ...
  if (ratio !== null) {
    ...
    if (price !== null) {
      ...
      footer = `Savings are a mean per-task estimate. Pricing for ${model}. Actual varies by task.`;
    } else {
      footer = 'Savings are a mean per-task estimate. Actual varies by task.';
    }
