...
### 9.B Typography
* **AVOID Inter as default.** See Section 4.1 (in `references/directives.md`). Override path exists.
...
* **Hand-rolled decorative SVGs strongly discouraged** as default (see Section 4.8 (in `references/directives.md`)).
...
* **NO `border-t` + `border-b` on every row of a long list / spec table.** Pick one (bottom-border between rows OR top-border above the group) and use it sparsely. A 10-row spec table with hairlines under each row is the laziest layout - see Section 4.9 (in `references/directives.md`) for alternative UI components.
