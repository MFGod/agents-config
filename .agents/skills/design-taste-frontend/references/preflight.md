...
- [ ] **Brief inference** declared (Section 0.B (in `SKILL.md`) one-liner)?
- [ ] **Dial values** explicit and reasoned from the brief, not silently using baseline?
- [ ] **Design system** chosen from Section 2 (in `SKILL.md`) if applicable, or aesthetic labeled honestly?
- [ ] **Redesign mode** detected and audit performed (if applicable, Section 11 (in `references/redesign.md`))?
- [ ] **ZERO em-dashes (`—`) anywhere on the page.** Headlines, eyebrows, pills, body, quotes, attribution, captions, buttons, alt text. Zero. (Section 9.G (in `references/ai-tells.md`) - non-negotiable.)
- [ ] **Page Theme Lock**: ONE theme (light, dark, or auto) for the whole page. No section flips to inverted mode mid-page (Section 4.11 (in `references/directives.md`))?
- [ ] **Color Consistency Lock**: one accent color used identically across all sections (Section 4.2 (in `references/directives.md`))?
- [ ] **Shape Consistency Lock**: one corner-radius system applied consistently (Section 4.4 (in `references/directives.md`))?
...
- [ ] **Long lists use the right UI component** (not default `<ul>` with `divide-y` for > 5 items - see Section 4.9 (in `references/directives.md`) alternatives)?
...
- [ ] **GSAP sticky-stack / horizontal-pan** implemented per Section 5.A (in `references/proactivity.md`) / 5.B canonical skeleton (`start: "top top"`, `pin: true`, correct scrub)?
...
- [ ] **No AI Tells** from Section 9 (in `references/ai-tells.md`) (Inter as default, AI-purple, three-equal cards, Jane Doe, Acme, "Quietly in use at")?
...
The sections below are vendored reference content. They give the agent real install commands, real canonical doc links, and real working starter snippets for each design system named in Section 2 (in `SKILL.md`). Use them to ground decisions in production reality, not training-data fiction.
