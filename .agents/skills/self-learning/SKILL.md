...
(this kit targets Claude Code and Cursor 2.4+, which both load `SKILL.md` skills
natively). Where a step differs by tool, the generic version comes first and any
...
- [ ] 3. **Dedupe.** Look for an existing skill to UPDATE rather than duplicate.
      ...
      one (Claude Code `.claude/skills` + `~/.claude/skills`, Cursor
...
- [ ] 7. **Prune on contradiction.** If this session proved an existing skill
      WRONG — its commands changed, its golden path no longer works, the tool it
      names is gone — say so explicitly and offer to fix or delete it. A skill
      that confidently describes a dead path is worse than no skill: the next
      session will trust it and lose time. Never delete without the user's OK.

### Scope: project vs global

- **Project** (the repo's skills directory — `.claude/skills/` or
  `.cursor/skills/`): the path is specific to THIS codebase — its env vars, its
...
- **Global** (your user-level skills directory — `~/.claude/skills/` or
  `~/.cursor/skills/`): the path generalizes across projects — a personal tool, a
...
- **Subagent** — if your tool can delegate to a subagent that **inherits this
  ...
  (Claude Code: a skill with `context: fork`.) Don't hand it to a *fresh* agent
  with no context — it would
...
- **Growth without pruning is the failure mode of this skill.** It only ever
  adds. Every skill's `description` sits in context every session, and
  overlapping descriptions make the agent pick the wrong one — so a skills
  directory that only grows eventually costs more than it saves. Harvest
  narrowly, and when a golden path dies, kill the skill that describes it
  (step 7). Periodic audit: see the kit's `CLAUDE.md`.
